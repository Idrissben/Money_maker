...
def map_rating_to_scale(
    rating: Union[str, float, None]
...
def preprocessing(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    for i, feature_i in enumerate(highly_correlated.columns):
        for j, feature_j in enumerate(highly_correlated.columns[:i]):
    ...
    dataset = pd.get_dummies(
        dataset, columns=["reportedCurrency"], prefix=["reportedCurrency"]
    )
    dataset = pd.get_dummies(dataset, columns=["symbol"], prefix=["symbol"])

    # Fill missing values with KNN Imputer
    imputer = KNNImputer(n_neighbors=5)
    dataset_preprocessed_imputed = pd.DataFrame(
        imputer.fit_transform(dataset), columns=dataset.columns
    )
    print("Applied KNNImputer.")

    # Scale columns, discarding currency and symbol columns.
    # Use robust scaler to limit outlier influence
    reported_columns = [
        col
        for col in dataset_preprocessed_imputed.columns
        if "reportedCurrency" in col or "symbol" in col
    ]
    columns_to_scale = [
        col
        for col in dataset_preprocessed_imputed.columns
        if col not in reported_columns
    ]
    scaler = RobustScaler()
    dataset_preprocessed_imputed_scaled = pd.DataFrame(
        scaler.fit_transform(dataset_preprocessed_imputed[columns_to_scale]),
        columns=columns_to_scale,
    )
    dataset_preprocessed_imputed_scaled = pd.concat(
        [
            dataset_preprocessed_imputed_scaled,
            dataset_preprocessed_imputed[reported_columns],
        ],
        axis=1,
    )
